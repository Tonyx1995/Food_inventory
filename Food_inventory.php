	...
	<body>
		<div id="container">
			<?php
				...
				$action = NULL;
				$page = 0;
				//Change this to how many items you want per page.
				$page_break_limit = 10;
				$offset = 0;
				...
				//Search/categorizing
				sortBy();
				//Just displaying the add new food and category buttons. (Logout button in here as well)
				addItem();
				...
				function delete($id){
					...
					$result = mysql_query($sql);
					//Way of directing users around without the headers already sent error.
					echo " 
						<script type='text/javascript'>
							window.location.href = 'food_inventory.php';
						</script>
					";
				}
					...
					if($num_rows > 0){
						...
						while($row = mysql_fetch_array($result)){
							...
							if($image){
								...
								echo '<td><img src ="Styles/pics/'.$default .'" height =100px width =100px></td>';
							}
							...
							if(($current_date >= $sale_start) && ($current_date <= $sale_end)){
								echo "<td style='font-weight: bold; color: red;'><img src='Styles/pics/sale.jpg' height='45px' width='45px' /><br /><s style='color: black; font-weight: normal;'>$".sprintf('%0.2f', $row['regular_price'])."</s> $".sprintf('%0.2f', $row['sale_price'])."</td>";
							}else{
							...
							if($row['cyclone_card_item'] == "1"){
								...
								//If sale price is lower, match it (checking to make sure item is on sale first)
							...
							//Edit Item
							echo"<td><form action=edit_food.php?id=".$row['food_id']." method=POST>
						...
						if(isset($_GET['page'])){
							...
							echo "<tr><p>";
							echo"&nbsp<a href='food_inventory.php?page=1'>First</a>&nbsp&nbsp&nbsp";
							if($page > 1){
								echo"&nbsp<a href='food_inventory.php?page=".($page-1)."'>&lt&lt</a>";
							}
							...
							while($row = mysql_fetch_array($result)){
								...
								for($x = max($page-2, 1); $x <= max(1, min($total_pages, $page+2)); $x++){
									echo "&nbsp<a href='Food_inventory.php?page=".$x."'>$x</a>";
								}		
							}
							if($page < $total_pages){
								echo" &nbsp<a href='food_inventory.php?page=".($page+1)."'>&gt&gt</a>";
							}
							echo" &nbsp&nbsp&nbsp&nbsp<a href='food_inventory.php?page=".$total_pages."'>Last</a>";
				 ...
				 echo'<table>
					<tr>
						...
						<form action="categories.php" method="post">
							<input type="submit" value="Category Maintenance" />
						</form>
						&nbsp&nbsp
						<form action="Food_inventory.php?logout=true" method="post">
